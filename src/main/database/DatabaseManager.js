...
class DatabaseManager {
  ...
  async searchSimilar(query, projectName = null, limit = 10) {
    // 这个方法需要在有 query embedding 的情况下使用
    // 实际使用时需要先通过 Ollama 生成 query 的向量
    const whereClause = projectName ? 'WHERE project_name = $2' : ''
    const params = projectName ? [query, projectName] : [query]
